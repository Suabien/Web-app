...
// manage3.js - JavaScript trang quản lý loại sản phẩm
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (addForm) {
        addForm.addEventListener('submit', function(e) {
            ...
            if (validateOptionForm(this)) {
                this.submit();
            }
        });
    }
    
    // Xử lý form sửa loại sản phẩm
...
function deleteOption(id) {
    if(confirm('Bạn có chắc chắn muốn xoá loại sản phẩm có ID ' + id + '?\nHành động này không thể hoàn tác.')) {
        fetch('OptionServlet', {
            method: 'POST',
        ...
        .then(response => {
            if(response.ok) {
                ...
                showNotification('❌ Có lỗi xảy ra khi xóa loại sản phẩm!', 'error');
            }
        })
        .catch(error => {
...
// Hàm mở modal chỉnh sửa loại sản phẩm
function openEditModal(id, option_name, price, quantity, image, product_id) {
    document.getElementById("edit-id").value = id;
    document.getElementById("edit-option-name").value = option_name;
    document.getElementById("edit-price").value = price;
    document.getElementById("edit-quantity").value = quantity;
    document.getElementById("edit-product-id").value = product_id || '';
    
    const preview = document.getElementById('edit-option-image-preview');
    const placeholder = document.getElementById('no-option-image-placeholder-edit');
